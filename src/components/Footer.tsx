      ...
      <footer className="border-t border-foreground/[0.06] py-12">
        <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
          ...
          <div className="flex flex-col gap-2">
            {["Accueil", "Avantages", "Offres", "Process", "FAQ", "Contact"].map((l) => (
              <a
                key={l}
                href={`#${l.toLowerCase() === "accueil" ? "hero" : l.toLowerCase()}`}
                className="font-body text-muted-foreground text-sm hover:text-foreground transition-colors"
              >
                {l}
              </a>
            ))}
        ...
        <div className="max-w-6xl mx-auto px-6 mt-8 pt-6 border-t border-foreground/[0.06]">
          <p className="font-body text-muted-foreground text-xs text-center">
            © 2026 Marc Dev — Tous droits réservés
          </p>
        </div>
      </footer>
